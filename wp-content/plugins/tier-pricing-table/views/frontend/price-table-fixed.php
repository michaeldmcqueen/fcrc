    ...
    <div class="price-rules-table-wrapper">
        ...
        <table class="shop_table price-rules-table <?php echo esc_attr( $settings['table_css_class'] ); ?>"
            ...
            <tr data-price-rules-amount="<?php echo esc_attr( $minimum ); ?>"
                ...
                <td>
					<?php if ( 1 >= array_keys( $price_rules )[0] - $minimum ) : ?>
                        <span><?php echo esc_attr( $minimum ); ?></span>
					<?php else : ?>
                        <span><?php echo esc_attr( $minimum ); ?> - <?php echo esc_attr( array_keys( $price_rules )[0] - 1 ); ?></span>
					<?php endif; ?>
                </td>
                <td>
			...
			<?php while ( $iterator->valid() ) : ?>
				...
				<?php
				do_action( 'tier_pricing_table_fixed_body_columns', $iterator, $price_rules, $real_price,
					$product_id );
				?>
