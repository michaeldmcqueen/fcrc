...
class THWCFE_Order_Data {
	...
	public function admin_order_data_after_order_details($order){
		...
		foreach($sections as $sname => $section_data){
			...
			if(is_array($fields) && !empty($fields)){
				...
				$html .= $this->get_order_meta_fields_html($order, $fields, self::VIEW_ADMIN_ORDER, $args);
			}
		}
	...
	public function display_custom_fields_in_order_details_page_customer($order){
		...
		foreach($sections as $sname => $section_data){
			...
			if(is_array($fields) && !empty($fields)){
				...
				$html .= $this->get_order_meta_fields_html($order, $fields, self::VIEW_CUSTOMER_ORDER);
			}
		}
	...
	private function display_custom_fields_in_emails($order, $sent_to_admin, $plain_text, $email){
		$html = '';
		$args = $this->prepare_args_admin_order_view($sent_to_admin);
		...
		foreach($sections as $sname => $section_data){
			...
			if(is_array($fields) && !empty($fields)){
				...
				$html .= $this->get_order_meta_fields_html($order, $fields, self::VIEW_ORDER_EMAIL, $args);
			}
		}
	...
	private function prepare_single_field_data($order_id, $name, $field, $context, $args){
		...
		if($type === 'label' || $type === 'heading'){
			...
			if($title || $subtitle){
				if($esc_attr_label){
					$title    = $title ? THWCFE_i18n::esc_attr__t($title) : '';
					$subtitle = $subtitle ? THWCFE_i18n::esc_attr__t($subtitle) : '';
				}else{
					$title    = $title ? THWCFE_i18n::t($title) : '';
					$subtitle = $subtitle ? THWCFE_i18n::t($subtitle) : '';
				}
			...
			if(!empty($value)){
				...
				$title = $esc_attr_label ? THWCFE_i18n::esc_attr__t($title) : THWCFE_i18n::t($title);
