...
abstract class WCFE_Checkout_Fields_Utils {
	...
	public function woo_email_customer_details($order, $sent_to_admin = false, $plain_text = false, $email = false){
		$settings = $this->get_advanced_settings();
		$position = $this->get_setting_value($settings, 'custom_fields_position_email');
		$html_enabled = $this->get_setting_value($settings, 'enable_html_in_emails') === 'yes' ? true : false;
	...
	public function woo_email_customer_details_fields($ofields, $sent_to_admin = false, $order = false){
		...
		if($position === 'woocommerce_email_customer_details_fields' && !$html_enabled){
			$ofields = $this->woo_display_custom_fields_in_emails($ofields, $sent_to_admin, $order);
		}
		return $ofields;
	}
	...
	public function woo_display_custom_fields_in_emails($ofields, $sent_to_admin, $order){
		$custom_fields = $this->get_custom_fields_for_display_in_emails($order, $sent_to_admin);
	...
	public function get_custom_fields_for_display_in_emails($order, $sent_to_admin){
		...
		foreach($sections as $sname => $section){
			if(!in_array($sname, $dis_sections)){
				$sfields = $this->get_section_fields_for_display_in_emails($order, $order_id, $section, $dis_fields, $sent_to_admin, $rfnames, $is_nl2br);
				if(is_array($sfields)){
					$custom_fields = array_merge($custom_fields, $sfields);
				}

				if($sname != 'billing' && $sname != 'shipping' && $sname != 'additional'){
					$rsfields = $this->get_repeat_section_fields_for_display_in_emails($order, $order_id, $sname, $section, $dis_fields, $sent_to_admin, $rsnames, $rfnames, $is_nl2br);
					if(is_array($rsfields)){
						$custom_fields = array_merge($custom_fields, $rsfields);
					}
				}
			}			
		}
	...
	public function get_repeat_section_fields_for_display_in_emails($order, $order_id, $key, $section, $dis_fields, $sent_to_admin, $rsnames, $rfnames, $is_nl2br=true){
		$custom_fields = array();
		...
		if(is_array($rsections)){
			foreach($rsections as $rskey => $rsection) {
				$fields = $this->get_section_fields_for_display_in_emails($order, $order_id, $rsection, $dis_fields, $sent_to_admin, $rfnames, $is_nl2br);
				if(is_array($fields)){
	...
	public function get_section_fields_for_display_in_emails($order, $order_id, $section, $dis_fields, $sent_to_admin, $rfnames, $is_nl2br=true){
		$custom_fields = array();
		...
		if(THWCFE_Utils_Section::is_valid_section($section) && THWCFE_Utils_Section::is_show_section($section, $cart_info)){
			...
			foreach($fields as $key => $field) {
				if(THWCFE_Utils_Field::is_custom_enabled($field) && !in_array($key, $dis_fields)){	
					$show_field = false;
					if($sent_to_admin && $field->get_property('show_in_email')){
						$show_field = true;					
					}else if(!$sent_to_admin && $field->get_property('show_in_email_customer')){
					...
					if($show_field){
						...
						if(THWCFE_Utils_Field::is_html_field($type)){
							$label = $key;
							$value = $field->get_property('title');
						...
						//Repeat Fields
						$rfields = $this->prepare_repeat_fields_for_emails($order_id, $key, $field, $rfnames);
						if(is_array($rfields)){
							$custom_fields = array_merge($custom_fields, $rfields);
						}		
					}
				}
			}
		...
		$custom_fields = $this->may_display_section_title($section, $custom_fields);
		return $custom_fields;
	}
